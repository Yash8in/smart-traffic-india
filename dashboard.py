...
if "route_inputs" not in st.session_state: st.session_state.route_inputs = {}
if "show_risk"    not in st.session_state: st.session_state.show_risk    = False
if "risk_inputs"  not in st.session_state: st.session_state.risk_inputs  = {}
...
# ════════════════════════════════════════════
# PAGE 3 — ROUTE RECOMMENDATION  (FIXED)
# ════════════════════════════════════════════
    ...
    if st.button("🔍 Find Best Route & Time", use_container_width=True):
        # Save all inputs to session state so results persist on re-run
        ...
        st.session_state.route_inputs = {
            ...
            "selected_snow":  selected_snow,
            "selected_cloud": selected_cloud,
        }

    if st.session_state.show_route and st.session_state.route_inputs:
        ri = st.session_state.route_inputs

        # Use saved inputs (not current widget values) so results don't disappear
        _source  = ri["source_city"]
        _dest    = ri["dest_city"]
        _hour    = ri["travel_hour"]
        _day     = ri["selected_day"]
        _month   = ri["selected_month"]
        _temp    = ri["selected_temp"]
        _rain    = ri["selected_rain"]
        _snow    = ri["selected_snow"]
        _cloud   = ri["selected_cloud"]

        src_info = CITIES[_source]
        dst_info = CITIES[_dest]
        dist_km  = int(((src_info["lat"]-dst_info["lat"])**2 +
                        (dst_info["lon"]-src_info["lon"])**2)**0.5 * 111)
        ...
        for r in route_options:
            h    = (_hour + r["hour_offset"]) % 24
            pred = predict_congestion(h, _day, _month, _temp, _rain, _snow, _cloud)
            route_results.append({**r, "pred": pred})
        route_results.sort(key=lambda x: x["pred"])
        best = route_results[0]

        st.markdown("---")
        st.markdown(f"### Results: **{_source}** → **{_dest}**")
        st.markdown(f"Departure at **{_hour}:00** on **{DAY_NAMES[_day]}**")

        # Map — built fresh each render using saved inputs
        m2 = folium.Map(
            location=[(src_info["lat"]+dst_info["lat"])/2,
                       (src_info["lon"]+dst_info["lon"])/2],
            zoom_start=6, tiles="CartoDB positron")

        src_pred = predict_congestion(_hour, _day, _month, _temp, _rain, _snow, _cloud)
        dst_pred = predict_congestion((_hour+2)%24, _day, _month, _temp, _rain, _snow, _cloud)

        folium.Marker([src_info["lat"], src_info["lon"]],
                      popup=f"<b>START: {_source}</b><br>Congestion: {congestion_label(src_pred)}",
                      tooltip=f"📍 {_source}",
                      icon=folium.Icon(color="blue", icon="play", prefix="fa")).add_to(m2)
        folium.Marker([dst_info["lat"], dst_info["lon"]],
                      popup=f"<b>END: {_dest}</b><br>Congestion: {congestion_label(dst_pred)}",
                      tooltip=f"🏁 {_dest}",
                      icon=folium.Icon(color="red", icon="flag", prefix="fa")).add_to(m2)

        route_colors = ["green", "orange", "blue"]
        ...
        st_folium(m2, width=1100, height=420, key="route_map")
        ...
        st.markdown("---")
        hour_preds = [predict_congestion(h, _day, _month, _temp, _rain, _snow, _cloud)
                      for h in range(24)]
...
# ════════════════════════════════════════════
    ...
    st_folium(m3, width=1100, height=420, key="alert_map")
...
# ════════════════════════════════════════════
    ...
    if st.session_state.show_risk and st.session_state.risk_inputs:
        ri = st.session_state.risk_inputs

        # Use saved inputs so results don't vanish on re-run
        _city       = ri["risk_city"]
        _hour       = ri["risk_hour"]
        _visibility = ri["visibility"]
        _road       = ri["road_type"]
        _rain       = ri["risk_rain"]
        _temp       = ri["risk_temp"]
        _day        = ri["selected_day"]
        ...
        pred  = predict_congestion(_hour, _day, _month, _temp, _rain, _snow, _cloud)
        score = accident_risk(_hour, _rain, _temp, pred)
        score += {"Clear":0,"Foggy":25,"Heavy Rain":30,"Snow/Ice":40}[_visibility]
        score += {"Highway":20,"City Road":10,"Local Street":5,"School Zone":15}[_road]
        score  = min(100, score)
        ...
        factors = []
        if 0 <= _hour <= 5:           factors.append("🌙 Late night driving")
        if _hour in [7,8,9,17,18]:    factors.append("🚦 Rush hour")
        if _visibility != "Clear":    factors.append("🌫️ Poor visibility")
        if _rain > 2:                 factors.append("🌧️ Wet roads")
        if _temp < 273:               factors.append("❄️ Freezing temperature")
        ...
        with c2:
            ...
            <div style='background:white; border-radius:12px; padding:20px;
                ...
                {'<br>'.join(factors) if factors else '✅ No major risk factors detected'}
            </div>""", unsafe_allow_html=True)
        ...
        # Risk map — always rendered from saved inputs
        st.markdown("### 🗺️ Risk Map — All Cities")
        m4 = folium.Map(location=[20.5937, 78.9629], zoom_start=5, tiles="CartoDB positron")
        for city, info in CITIES.items():
            cp        = predict_congestion(_hour, _day, _month, _temp, _rain, _snow, _cloud)
            cs        = accident_risk(_hour, _rain, _temp, cp)
            cs       += {"Clear":0,"Foggy":25,"Heavy Rain":30,"Snow/Ice":40}[_visibility]
            cs       += {"Highway":20,"City Road":10,"Local Street":5,"School Zone":15}[_road]
            cs        = min(100, cs)
        ...
        st_folium(m4, width=1100, height=420, key="risk_map")
...
# ════════════════════════════════════════════
    ...
    st_folium(m5, width=1100, height=500, key="hotspot_map")
